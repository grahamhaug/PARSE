...
"""
- Perform 100k runs selecting 6 sulfinates via SRS
- Calculate min/avg/max pairwise distance between selected sulfs for each run
- Output the calc data to a summary csv
- Reads in a properties csv containing SMILEs, MW, FSP3, Cm data
"""

#import set of sulfinate residues containing unscaled calc. props
import_file_name = 'properties_sulfinates.csv'
...
while i < 10001:
    ...
    random_selection = incoming_data.sample(n=6, replace=False, weights=None)
...
outData['PD_Max'] = maxs
outData['PD_Avg'] = avgs
outData.to_csv(f'Random_Sampling_Performance.csv', index=False, header=True)
