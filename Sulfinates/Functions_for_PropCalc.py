...
def write_to_sdf(smiles_list):

	chunked_list = process_in_batches(smiles_list)	
	num_chunks = len(chunked_list)
	print(f'\nWriting mol data to {num_chunks} SD files...')
	batch_num = 1
	for chunk in chunked_list:
		chunk_mols = []
		#for every smile of the <10k smiles in chunk, convert smile to a mol
		...
		print(f'\t\tNumber of mols in SD file: {len(chunk_mols)}')

		#write the mols to a SD file
		with Chem.SDWriter(f'Sulfinates_chunk{batch_num}_{len(chunk_mols)}mols.sdf') as w:
			for mol in chunk_mols:
				w.write(mol)
		w.close()	
