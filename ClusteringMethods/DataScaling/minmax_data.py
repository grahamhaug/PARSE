...
#apply minmax scaling to numerical column data
min_max_scaler = MinMaxScaler()
# std_scaler

#create and output formatted csv
df_std = pd.DataFrame(min_max_scaler.fit_transform(num_df), columns=num_df.columns)
output_df = pd.concat([header_df, df_std], axis=1)
output_df.to_csv(f'MinMaxscaled_sulfinates.csv', index=False, header=True)
