...
#perform k iterations of K-means clustering
#outputs k files containing the molecules closest to cluster centroids
s_scores = []
num_runs = 20
for k in range(1,num_runs+1):
    ...
    kmeans = kmeans.fit(prepped_data)
    ...
    plt.legend(loc='best')
    plt.savefig(f'kmeans_k{clusters}_standardized_sulfinates_{k}.png', bbox_inches="tight")

#evaluate pairwise distances between selected sulfinates
#also output SD file of the 
mins = []
maxs = []
avgs = []
iterate = []
...
outData['PD_Max'] = maxs
outData['PD_Avg'] = avgs
outData.to_csv(f'KMeans_Sampling_Performance-standardized.csv', index=False, header=True)