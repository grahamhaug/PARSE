...
#Make sure a candidate fragment actually contains a carboxylic acid
...
decharged_structures = remove_charged_smiles(processed_structures)

#remove any molecules over 1000 mw
#also checks that SMILES can be converted to RDkit Mol objects
#(sometimes not the case)
weighted_structures = remove_high_mw(decharged_structures, 1000)
...
smarts_structures = look_for_smarts(weighted_structures)

#canonicalize the remaining SMILES
#drop duplicates; outputs a .csv in local dir
candidate_structures = canonicalize_smiles(smarts_structures,f'processed_{chop_name}')
